            ...
            {(signals ?? []).map((s) => (
              <tr key={s.id} className="border-b border-border/40 hover:bg-secondary/30 transition-colors">
                <td className="px-4 py-2.5 max-w-xs">
                  <div className="truncate text-foreground">{s.market_question}</div>
                </td>
                ...
                <td className="px-3 py-2.5 text-center">
                  <span className={cn("font-bold", s.direction === "YES" ? "text-[hsl(var(--success))]" : "text-destructive")}>
                    {s.direction}
                  </span>
                </td>
                ...
                <td className="px-3 py-2.5 text-right">
                  <span className={cn("font-bold tabular-nums", s.edge > 0 ? "text-[hsl(var(--success))]" : "text-destructive")}>
                ...
                <td className="px-3 py-2.5 text-right text-[10px] text-muted-foreground">
                  {new Date(s.detected_at).toLocaleString("en-US", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit", hour12: false })}
                </td>
              </tr>
            ))}
          </tbody>
