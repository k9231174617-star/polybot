    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className={cn(
          ...
          {paperOn ? t("paper_badge_on") : t("paper_badge_off")}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <StatCard
          label={t("paper_cum_pnl")}
          value={`${pnlPositive ? "+" : ""}$${(stats?.cumulative_pnl ?? 0).toFixed(2)}`}
          positive={pnlPositive}
        />
        <StatCard
          label={t("paper_return")}
          value={`${returnPositive ? "+" : ""}${(stats?.total_return_pct ?? 0).toFixed(2)}%`}
          ...
          {chartData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                />
                <Area type="monotone" dataKey="pnl" stroke="hsl(var(--primary))"
                  fill="url(#paperGrad)" strokeWidth={2} dot={false} name="P&L" />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </div>
            ...
            {Object.entries(byType).map(([type, data]) => (
              <div key={type} className="border border-border rounded-md p-3 space-y-1.5">
                ...
                <div className="grid grid-cols-3 gap-1 text-[10px] text-muted-foreground">
                  ...
                  <div>WR: <span className={cn("font-mono", (data.win_rate ?? 0) >= 0.5 ? "text-success" : "text-destructive")}>
                    {((data.win_rate ?? 0) * 100).toFixed(0)}%
                  </span></div>
                  <div>P&L: <span className={cn("font-mono", (data.pnl ?? 0) >= 0 ? "text-success" : "text-destructive")}>
      ...
      <div className="bg-card border border-border rounded-md overflow-hidden">
        <div className="px-4 py-3 border-b border-border text-xs font-semibold uppercase tracking-wider text-muted-foreground">
          {t("paper_positions")} ({positions.length})
        </div>
        {positions.length === 0 ? (
              ...
              <tbody>
                {positions.map((p) => (
                  <tr key={p.id} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                    <td className="px-4 py-2 max-w-[180px] truncate font-medium" title={p.market_question}>
                      {p.market_question}
                    </td>
                    <td className="px-3 py-2">
                      <span className={cn("font-mono text-[10px] px-1.5 py-0.5 rounded",
                        p.side === "YES" ? "bg-success/20 text-success" : "bg-destructive/20 text-destructive"
                      )}>{p.side}</span>
                    </td>
                    ...
                    <td className={cn("px-3 py-2 text-right font-mono",
                      p.unrealized_pnl >= 0 ? "text-success" : "text-destructive"
                    )}>
                      {p.unrealized_pnl >= 0 ? "+" : ""}${p.unrealized_pnl.toFixed(2)}
                      <span className="text-muted-foreground ml-1">({p.unrealized_pnl_pct.toFixed(1)}%)</span>
                    </td>
              ...
              <tbody>
                {trades.map((tr) => (
                  <tr key={tr.id} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                    <td className="px-4 py-2 max-w-[160px] truncate font-medium" title={tr.market_question}>
                      {tr.market_question}
                    </td>
                    <td className="px-3 py-2">
                      <span className={cn("font-mono text-[10px] px-1.5 py-0.5 rounded uppercase",
                        tr.action === "buy" ? "bg-primary/20 text-primary" : "bg-secondary text-muted-foreground"
                      )}>{tr.action}</span>
                    </td>
                    <td className="px-3 py-2">
                      <span className={cn("font-mono text-[10px] px-1.5 py-0.5 rounded",
                        tr.side === "YES" ? "bg-success/20 text-success" : "bg-destructive/20 text-destructive"
                      )}>{tr.side}</span>
                    </td>
                    ...
                    <td className={cn("px-3 py-2 text-right font-mono",
                      tr.realized_pnl == null ? "text-muted-foreground"
                        : tr.realized_pnl >= 0 ? "text-success" : "text-destructive"
                    )}>
                      {tr.realized_pnl == null ? "—" : `${tr.realized_pnl >= 0 ? "+" : ""}$${tr.realized_pnl.toFixed(2)}`}
                    </td>
                    <td className="px-3 py-2 text-[10px] text-muted-foreground font-mono">
                      {tr.signal_type.replace(/_/g, " ")}
                    </td>
