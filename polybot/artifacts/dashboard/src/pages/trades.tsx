    ...
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        ...
        <div className="flex gap-4 ml-auto font-mono text-xs">
          <div className="text-muted-foreground">{t("realized_pnl")}:
            <span className={cn("ml-1 tabular-nums", totalRealizedPnl >= 0 ? "text-[hsl(var(--success))]" : "text-destructive")}>
              ...
              return (
                <tr key={tr.id} className="border-b border-border/40 hover:bg-secondary/30 transition-colors">
                  <td className="px-4 py-2.5 max-w-xs">
                    <div className="truncate text-foreground">{tr.market_question}</div>
                  </td>
                  <td className="px-3 py-2.5 text-center">
                    ...
                    )}>{tr.action.toUpperCase()}</span>
                  </td>
                  <td className={cn("px-3 py-2.5 text-center font-bold",
                    tr.side === "YES" ? "text-[hsl(var(--success))]" : "text-destructive")}>
                    {tr.side}
                  </td>
                  ...
                  <td className="px-3 py-2.5 text-right tabular-nums text-muted-foreground">
                    {tr.slippage ? `${(tr.slippage * 100).toFixed(2)}%` : "—"}
                  </td>
                  ...
                  <td className="px-3 py-2.5 text-right">
                    {pnl !== undefined && pnl !== null ? (
                      <span className={cn("tabular-nums font-bold", pnl >= 0 ? "text-[hsl(var(--success))]" : "text-destructive")}>
                        {pnl >= 0 ? "+" : ""}${pnl.toFixed(2)}
                      </span>
                    ) : <span className="text-muted-foreground">—</span>}
                  </td>
                  <td className="px-3 py-2.5 text-right text-muted-foreground">
                    <span className="text-[10px] uppercase">{tr.order_type}</span>
                  </td>
                  <td className="px-3 py-2.5 text-right text-[10px] text-muted-foreground">
                    {new Date(tr.executed_at).toLocaleString("en-US", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit", hour12: false })}
                  </td>
                </tr>
              );
            })}
